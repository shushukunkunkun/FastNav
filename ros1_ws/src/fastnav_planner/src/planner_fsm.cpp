...
// 从当前状态重新规划；当前 path 表达没有连续 B-spline 时间状态，因此起点仍取最新 odom。
...
bool PlannerFSM::callReplan(bool /*use_current_path*/, bool /*use_random_init*/)
{
    ...
    getLocalTarget();
    const bool success = planner_manager_->planToGoal(local_target_pt_);
...
// 发布当前路径和 A* 搜索节点。路径消息仍保持 nav_msgs::Path，不绑定 B-spline。
