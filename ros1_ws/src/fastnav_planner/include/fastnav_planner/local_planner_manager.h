 ...
 * 3. 持有 PathOptimizer，对 A* 原始路径做 shortcut 等轻量后处理；
...
private:
    ...
    // 将 A* / optimizer 输出的离散点列参数化成 UniformBspline，并写入 local_data_。
    void updateTrajInfo(const std::vector<Eigen::Vector3d>& path,
                        const ros::Time& time_now);

    // 用路径点构造 EGO 风格全局多项式参考轨迹，并写入 global_data_。
    void updateGlobalTrajInfo(const std::vector<Eigen::Vector3d>& path,
                              const ros::Time& time_now);

    // 把离散点列参数化成三次均匀 B 样条，当前先使用零首尾速度 / 加速度约束。
    bool buildUniformBsplineFromPath(const std::vector<Eigen::Vector3d>& path,
                                     ego_planner::UniformBspline& traj) const;

    // 根据路径长度和 nominal velocity 估计轨迹总时长 $T=\sum_i\|p_{i+1}-p_i\|/v$。
    double estimatePathDuration(const std::vector<Eigen::Vector3d>& path) const;
...
private:
    ...
    // B-spline 维护参数。当前用路径点作为控制点，先建立可采样轨迹数据结构。
    int bspline_degree_{3};
    double traj_nominal_vel_{1.0};
