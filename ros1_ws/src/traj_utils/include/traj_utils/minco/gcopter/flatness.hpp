/*
 * FastNav 注释：flatness.hpp 实现多旋翼微分平坦映射。
 * 对四旋翼，位置轨迹 $p(t)$ 和 yaw 可作为平坦输出，给定 $\dot p$、$\ddot p$、$p^{(3)}$ 后可恢复推力、姿态和角速度。
 * 代码中考虑阻力模型，等效加速度不只是 $\ddot p+g e_z$，还会加入与速度相关的水平、竖直和寄生阻力项。
 * forward() 的核心输出是 thrust、quat、omg，用于检查 $f_{min}\le f\le f_{max}$、$\theta\le\theta_{max}$ 和 $\|\omega\|\le\omega_{max}$。
 * backward() 系列函数把动力学代价对 thrust/quat/omg 的梯度反传回轨迹导数，用于 L-BFGS 优化。
 * 如果 FastNav 先使用点质量模型，可以暂时只用速度/加速度约束；要做更真实的无人机约束时再接本文件。
 */
