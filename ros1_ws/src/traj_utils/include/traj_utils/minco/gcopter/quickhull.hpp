...
/*
 * FastNav 注释：quickhull.hpp 实现 QuickHull 凸包算法。
 * 给定点集 $\{p_i\}$，算法构造包含所有点的最小凸多面体边界，输出面片和邻接关系。
 * 在安全走廊中，凸包常用于把点/顶点集合转成多面体边界，辅助生成 $V$ 表示或可视化。
 * 文件较长且包含大量几何细节，核心思想是递归寻找距离当前面最远的点并扩展可见面。
 */
