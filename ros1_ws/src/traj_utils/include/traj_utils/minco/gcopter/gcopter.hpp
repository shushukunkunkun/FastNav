/*
 * FastNav 注释：gcopter.hpp 是完整 GCOPTER 优化器，负责把 MINCO、凸安全走廊、动力学约束和 L-BFGS 组合起来。
 * GCOPTER_PolytopeSFC 以安全飞行走廊 $\mathcal{P}_k=\{x\mid H_k x\le h_k\}$ 约束轨迹位置，使采样点尽量落在对应凸多面体内。
 * 优化变量包括内部路径点、每段时间以及多面体内点的参数化变量；时间通过正值映射 $T=f(\tau)>0$ 保证每段时长为正。
 * 代价包含平滑能量、总时间项 $\rho T$，以及速度、角速度、倾角、推力等约束违反惩罚。
 * FlatnessMap 把 $\dot p$、$\ddot p$、$p^{(3)}$ 映射到推力、姿态和角速度，用于动力学约束评估。
 * optimize() 调用 L-BFGS，在每次迭代中先用 MINCO 由变量恢复 Trajectory<5>，再积分评估约束代价和梯度。
 * 对 FastNav 来说，本文件属于“完整优化器层”，应在我们已有 A* 和局部 voxel map 稳定后再逐步接入。
 */
