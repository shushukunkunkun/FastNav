...
/*
 * FastNav 注释：firi.hpp 实现 FIRI，即 Fast Iterative Region Inflation，用于生成凸安全区域。
 * 给定一段路径和障碍点云，FIRI 迭代构造半空间 $a_i^T x\le b_i$，让凸多面体尽量大且不包含障碍点。
 * 其几何意义是沿参考线段膨胀一个安全走廊，使轨迹只要满足 $H x\le h$ 就位于无碰区域内。
 * 内部会使用线性规划和 L-BFGS 来优化椭球/多面体关系，常见目标是最大化区域体积或扩大可行余量。
 * 在 FastNav 中，FIRI 可以作为“局部 A* 路径 -> 安全飞行走廊”的桥接模块，但当前 voxel map 阶段可以先不接。
 */
...
namespace firi
{
    ...
    inline void chol3d(const Eigen::Matrix3d &A,
                       Eigen::Matrix3d &L)
    {
        // 手写 3x3 Cholesky：求 $A=LL^T$。FIRI 用它把椭球矩阵 $Q$ 转成下三角参数，方便优化。
        L(0, 0) = sqrt(A(0, 0));
        L(0, 1) = 0.0;
        L(0, 2) = 0.0;
    ...
    inline bool smoothedL1(const double &mu,
                           ...
                           double &df)
    {
        // 平滑 hinge/L1 惩罚。只有约束违反 $x\ge0$ 时才产生代价，$\mu$ 控制 $x=0$ 附近的光滑过渡。
        if (x < 0.0)
        {
            return false;
        }
                           ...
                           Eigen::VectorXd &grad)
    {
        ...
        // 约束被归一化成 $\|A_iL\|+A_ip\le1$，表示椭球 $E=\{p+Lu:\|u\|\le1\}$ 完全位于半空间内。
        ...
        // 最大体积内接椭球等价于最大化 $\det(L)$，所以代价中加入 $-\log\det(L)$。
                                   ...
                                   Eigen::Vector3d &r)
    {
        ...
        // 先解线性规划找最深内点，目标可理解为最大化到所有归一化平面的最小距离 $d$。
        ...
        // 若 maxdepth <= 0，说明半空间交集没有有效内点，无法继续求内接椭球。
        ...
        *pM = M;
        ...
        Eigen::VectorXd x(9);
        // 当前椭球写作 $E=\{p+R\,diag(r)u:\|u\|\le1\}$，等价形状矩阵为 $Q=R\,diag(r^2)R^T$。
                     ...
                     const double epsilon = 1.0e-6)
    {
        ...
        // FIRI 的输入线段端点是 a,b。输出 hPoly 是半空间集合 $h_i^T[x,y,z,1]^T\le0$，需要保证 a,b 都在初始 box bd 内。
        ...
        // 初始化椭球中心为线段中点，方向为单位阵，半轴为 1；后续每轮用当前椭球诱导新的分离平面。
        ...
        {
            // forward 把世界坐标映射到当前椭球归一化空间：$y=diag(1/r)R^T(x-p)$。
            // 在该空间里当前椭球变成单位球 $\|y\|\le1$，障碍点的切平面更容易构造。
            ...
            for (int i = 0; i < N; i++)
            {
                // 对每个障碍点 $q_i$，在归一化空间中构造单位球的切平面 $n_i^Ty-d_i\le0$，用于把障碍点隔在多面体外。
                ...
                {
                    // 如果切平面把线段端点 a 排除在外，就把平面绕到同时包含 a 的方向，保证线段始终在安全区域内。
            ...
            // 每轮贪心加入最近的边界平面或障碍切平面；加入一个平面后，会删除已经被该平面隔开的障碍点。
            ...
            for (int i = 0; i < nH; ++i)
            {
                // 把归一化空间中的半空间变回世界坐标：若 $\bar h^T[y,1]^T\le0$ 且 $y=F(x-p)$，则 $h_x=\bar h_xF$，$h_3=\bar h_3-h_xp$。
            ...
            // 用当前多面体求最大体积内接椭球，下一轮再基于这个更大的椭球构造更好的分离平面。
