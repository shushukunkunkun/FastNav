...
/*
 * FastNav 注释：trajectory.hpp 定义 GCOPTER/MINCO 的分段多项式轨迹容器。
 * Piece<D> 表示单段 $D$ 次多项式，内部保存 $p(t)=\sum_{i=0}^{D} c_i t^i$ 的三维系数矩阵。
 * Trajectory<D> 是 Piece<D> 的有序序列，整条轨迹总时长为 $T=\sum_k T_k$。
 * getPos/getVel/getAcc/getJer 会先定位 $t$ 所在分段，再在局部时间上计算 $p(t)$、$\dot p(t)$、$\ddot p(t)$、$p^{(3)}(t)$。
 * checkMaxVelRate/checkMaxAccRate 通过求解 $\|\dot p(t)\|^2-v_{max}^2=0$ 和 $\|\ddot p(t)\|^2-a_{max}^2=0$ 的根来判断整段是否超限。
 * 对 FastNav 来说，后续若采用 MINCO，本文件中的 Trajectory<5> 可以作为 planner 与 controller 之间的标准轨迹表达。
 */
