...
/*
 * FastNav 注释：sdlp.hpp 实现 Seidel 低维线性规划，用于快速求解小维度 LP。
 * 典型问题形式是在线性约束 $a_i^T x\le b_i$ 下优化线性目标 $c^T x$。
 * FIRI 和多面体几何计算会频繁查询某个方向上的最远点、可行性或支撑平面，低维 LP 能保持很高效率。
 * 对 FastNav 来说，它是安全走廊生成的底层几何依赖，不属于轨迹表达本身。
 */
