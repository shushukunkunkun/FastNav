...
/*
 * FastNav 注释：minco.hpp 实现非均匀时间 MINCO 参数化。
 * MINCO 的核心思想是把完整多项式系数 $c$ 消元为中间点 $q$ 和每段时间 $T$，从而减少优化变量数量。
 * MINCO_S2NU 生成三次轨迹，对应边界状态 $[p,v]$；MINCO_S3NU 生成五次轨迹，对应边界状态 $[p,v,a]$；MINCO_S4NU 生成七次轨迹，对应 $[p,v,a,j]$。
 * setConditions() 固定首尾状态和分段数，setParameters() 输入内部点和时间，随后通过带状线性系统 $A c=b$ 求出所有多项式系数。
 * BandedSystem 用带状 LU 分解求解 $A c=b$，复杂度接近 $O(N)$，其中 $N$ 是分段数。
 * getEnergy() 计算平滑能量，例如 S3 通常对应 $\int_0^T \|p^{(3)}(t)\|^2dt$，也就是 jerk 平方积分。
 * getEnergyPartialGradByCoeffs()/getEnergyPartialGradByTimes() 给出能量对系数和时间的梯度，propogateGrad() 再反传到中间点和时间变量。
 */
