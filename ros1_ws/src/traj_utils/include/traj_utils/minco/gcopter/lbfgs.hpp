...
/*
 * FastNav 注释：lbfgs.hpp 是 header-only L-BFGS 优化器。
 * L-BFGS 用有限历史的 $s_k=x_{k+1}-x_k$ 和 $y_k=\nabla f_{k+1}-\nabla f_k$ 近似 Hessian 逆，避免显式存储大矩阵。
 * 接口 lbfgs_optimize() 每次回调用户代价函数，要求返回 $f(x)$ 和梯度 $\nabla f(x)$。
 * GCOPTER 用它优化 MINCO 的中间点、时间和走廊参数；FastNav 以后做 MINCO 轨迹优化也可以直接复用。
 */
